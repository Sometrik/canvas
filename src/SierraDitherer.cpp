...
SierraDitherer::apply() {  
  ...
  for (int y = 0; y < int(height); y++) {
	...
	*output_data++ = PACK_RGB565(r, g, b);
      ...
      addError(x + 1, y,     5.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x + 2, y,     3.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x - 2, y + 1, 2.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x - 1, y + 1, 4.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x,     y + 1, 5.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x + 1, y + 1, 4.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x + 2, y + 1, 2.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x - 1, y + 2, 2.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x,     y + 2, 3.0f/32.0f, r_error, g_error, b_error, a_error);
      addError(x + 1, y + 2, 2.0f/32.0f, r_error, g_error, b_error, a_error);
    }
  }
