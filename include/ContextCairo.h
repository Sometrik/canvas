...
namespace canvas {
  ...
  public:
    ...
    CairoSurface(unsigned int _logical_width, unsigned int _logical_height, unsigned int _actual_width, unsigned int _actual_height, const InternalFormat & image_format);
    ...
    void renderPath(RenderMode mode, const Path2D & path, const Style & style, float lineWidth, Operator op, float display_scale, float globalAlpha, float shadowBlur, float shadowOffsetX, float shadowOffsetY, const Color & shadowColor, const Path2D & clipPath);
    ...
    void drawImage(Surface & _img, double x, double y, double w, double h, float displayScale, float globalAlpha, float shadowBlur, float shadowOffsetX, float shadowOffsetY, const Color & shadowColor, const Path2D & clipPath, bool imageSmoothingEnabled = true);
    void drawImage(const Image & _img, double x, double y, double w, double h, float displayScale, float globalAlpha, float shadowBlur, float shadowOffsetX, float shadowOffsetY, const Color & shadowColor, const Path2D & clipPath, bool imageSmoothingEnabled = true);
  ...
  protected:
    ...
    void drawNativeSurface(CairoSurface & img, double x, double y, double w, double h, float alpha, const Path2D & clipPath, bool imageSmoothingEnabled);
  ...
  public:
  ContextCairo(unsigned int _width, unsigned int _height, const InternalFormat & image_format, float _display_scale = 1.0f)
    ...
    std::shared_ptr<Surface> createSurface(unsigned int _width, unsigned int _height, const InternalFormat & format) {
  ...
  public:
    CairoContextFactory() { }
    std::shared_ptr<Context> createContext(unsigned int width, unsigned int height, const InternalFormat & image_format, bool apply_scaling) { return std::shared_ptr<Context>(new ContextCairo(width, height, image_format)); }
    std::shared_ptr<Surface> createSurface(const std::string & filename) { return std::shared_ptr<Surface>(new CairoSurface(filename)); }
    std::shared_ptr<Surface> createSurface(unsigned int width, unsigned int height, const InternalFormat & image_format, bool apply_scaling) {
